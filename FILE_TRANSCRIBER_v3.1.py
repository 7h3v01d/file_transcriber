"""
╔══════════════════════════════════════════════════════════╗
║   FILE TRANSCRIBER v3.0  by Leon Priest                  ║
...
def call_ollama_api(prompt, base_url, model):
    import json as _json
    # Use the OpenAI-compatible /api/chat endpoint
...
def fetch_ollama_models(base_url):
    """Return list of model names available in Ollama, or empty list on error."""
    import json as _json
    try:
...
class FileTranscriberApp:
    ...
    def _build_right_panel(self, body):
        ...
        right.grid(row=1, column=2, sticky="nsew", pady=8)
        ...
        tk.Label(right, text="FILE PREVIEW", font=("Courier", 9, "bold")).grid(
            row=4, column=0, columnspan=4, sticky="w", pady=(12, 4))
        ...
        self.preview_label = tk.Label(right, text="", font=("Courier", 8), anchor="w")
        self.preview_label.grid(row=6, column=0, columnspan=4, sticky="ew", pady=(2, 0))
